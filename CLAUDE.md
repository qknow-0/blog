...
- **source-read 源码阅读**：新增源码阅读项目时，按 `source-read/README.md` 的规范操作——clone 源码到 `source-read/<项目名>/`，创建同名笔记文件夹，同时在 `.gitignore` 和 `backup.sh` 中添加排除。每篇文章必须包含两层内容：（1）**整体架构学习**——模块划分、数据流、设计决策 WHY；（2）**优秀代码学习**——摘录具体代码、标注好在哪、提炼骨架代码、警示反模式；（3）**Prompt 提取**——如果源码中有 LLM prompt 模板，单独写一篇中英双语 prompt 全集
