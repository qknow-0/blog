# QuantDinger 源码阅读（七）：基础设施——Docker 部署、认证计费与安全设计

> 最后一篇看基础设施层——Docker Compose 编排、认证计费和安全设计。这一篇是整个系列的收束。——Docker Compose 怎么编排五个服务、多用户认证和角色体系怎么设计、三种计费模式（credits/membership/USDT）怎么切换、以及贯穿整个项目的安全底线在哪里。
