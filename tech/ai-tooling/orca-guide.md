...
Orca 使用 **Ghostty** 作为终端引擎——WebGL 渲染，无限分屏，滚动记录在重启后仍然保留。快捷键继承 Ghostty 的默认键位。
...
| 操作        | 快捷键                                  |
| --------- | ------------------------------------ |
| 切到工作区 1-9 | `⌘1` ~ `⌘9`（可在 Settings 里改终端和全局的优先级） |
| 上/下一个工作区  | `⌘⌥↑` / `⌘⌥↓`                        |
| 新建工作区     | `⌘N`                                 |
| 快速创建工作区   | `⌘ShiftN`                            |
| 打开项目      | `⌘ShiftO`                            |
...
App 级快捷键可在 `Settings → Keyboard Shortcuts`（`⌘/`）里自定义。终端快捷键可改 Ghostty 配置 `~/.config/ghostty/config`。
