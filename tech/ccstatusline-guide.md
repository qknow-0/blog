...
一个典型的 Git 密集开发者的配置示例：

```
┌─ Model: Claude Sonnet 4.6 ─ main ─ +42 -3 ─ PR #233 (Open) ─ feat/add-auth ────── 18.6k ctx ─┐
```

需要的部件：
- **Model**：显示当前模型
- **Git Branch**：当前分支名
- **Git Insertions / Deletions**：未提交的 +42/-3 变更
- **Git PR**：当前分支的 PR 状态
- **Git Root Dir**：仓库根目录名
- **Context %**：上下文用量百分比
...
如果你是重度 Claude Code 用户，关心 API 消耗：

```
┌─ 4.2k in ─ 8.1k out ─ 2.8k cached ─ Speed: 156 tok/s ─ Ctx: 45.3k/200k (22.6%) ─ $0.42 ── 3hr 12m ─┐
```

需要的部件：
- **Tokens Input / Output / Cached**：输入输出缓存计数
- **Input Speed / Output Speed**：实时 Token 速度
- **Context Bar**：上下文进度条
- **Session Cost**：会话花费
- **Block Timer**：当前块已用时间
...
ccstatusline v2.0 引入了 Powerline 模式，效果如下：

```
╭─ Model: Sonnet 4.6 ── main ── 42.3k ctx ── Block: 2hr 15m ── Session: 1hr 30m ─╮
```

步骤：
1. 在 TUI 中选择 Powerline Setup 开启
2. 选择内置主题或自定义颜色
3. 按 `a` 开启多行自动对齐
4. 按需要自定义端帽（separator cap）字符

Powerline 模式下支持自动安装 Nerd Font，确保箭头字符可以正确渲染。

### 场景五：自定义命令集成

利用 **Custom Command** 部件，你可以将任何命令输出嵌入状态行：
...
- 按 `e` 编辑命令
- 按 `w` 设置最大宽度
- 按 `t` 设置超时时间（默认 1000ms）
- 按 `p` 切换是否保留 ANSI 颜色

> **注意**：命令应当快速执行，超时后会被自动终止。
