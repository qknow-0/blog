# 技术笔记

框架、工具相关的学习和实践记录。

## 分类

- [AI 编程工具链](ai-tooling/index.md) — 28 篇，AI 辅助编程的工具与框架
- [Claude Code](claude-code/index.md) — 4 篇，Claude Code 配置、插件与生态
- [开发工具](dev-tools/index.md) — 27 篇，开发环境与基础设施
- [容器化系列](containerization/index.md) — 5 篇，Docker → Compose → K8s → Helm
- [Robot Framework](robotframework/index.md) — 1 篇，源码阅读
- [逆向工具链](reversing/index.md) — 6 篇，闭源应用行为分析工具
- [AI Agent 系列](ai-agent/index.md) — 13 篇，从零构建 LLM Agent + LLM 训练原理
- [HTTP 协议系列](http/index.md) — 9 篇，从请求响应到 HTTPS，再到 API 设计三选一
- [Kafka 系列](kafka/index.md) — 10 篇，消息队列核心机制到生产实践
- [网络底层系列](network/index.md) — 7 篇，TCP/IP 传输层与网络层
- [NewsNow](newsnow/index.md) — 1 篇，新闻聚合器架构分析
- [向量数据库系列](vector-db/index.md) — 4 篇，ChromaDB / Milvus / pgvector
- [Neo4j](neo4j/index.md) — 1 篇，图数据库实战
